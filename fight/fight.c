...
void	draw_in_battle(t_game *game)
{
	...
	text = TTF_RenderText_Blended(font, "Attack", color);
	 SDL_BlitSurface(text, sdlh_rect(0, 0, 512,512),
		game->screen, sdlh_rect(200, 300, 0, 0)
		);
	SDL_FreeSurface(text);
	TTF_CloseFont(font);
}

void	fight_bg(t_game *game)
{ /* @TODO refactor & finish */
	...
	if (!g_bg)
		g_bg = ximg_load("res/battle_bg.png"), g_bg_text = ximg_load("res/battle_text.png"),
		warrior = ximg_load("res/warrior.gif"), ninja = ximg_load("res/ninja.gif");

	SDL_BlitSurface(g_bg, sdlh_rect(0, 0, 512, 341),
		game->screen, sdlh_rect(0, 0, 0, 0)
	);
	SDL_BlitSurface(g_bg_text, sdlh_rect(0, 0, 512, 171),
		game->screen, sdlh_rect(0, 341, 0, 0)
	);
	SDL_BlitSurface(warrior, sdlh_rect(0, 0, 512, 512),
		game->screen, sdlh_rect(50, 75 , 0, 0)
		);
	SDL_BlitSurface(ninja, sdlh_rect(0, 0, 512, 512),
		game->screen, sdlh_rect(400, 75 , 0, 0)
		);
	draw_in_battle(game);
}
